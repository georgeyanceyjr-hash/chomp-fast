// Experimental V7 candidate based on validated V6; see speed-report.md for validation scope.
...
struct ClassShard {
    ...
    bool maybe_hashed(uint64_t z)const{
        ...
        size_t blocks=bloom_bits>>9,base=(size_t(z)&(blocks-1))<<3;
...
int main(int argc,char**argv){
 try{
    if(argc<2){std::cerr<<"usage: chomp_sieve_v7_candidate ppositions.bin [max-dimension] [checkpoint] [preload-checkpoint] [only-width] [only-height] [threads]\n";return 2;}
